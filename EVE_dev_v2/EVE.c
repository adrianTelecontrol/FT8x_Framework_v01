...
void EVE_Write32(uint32_t data) {
  // if (dlBufferIndex + 4 > (uint16_t)DL_BUFFER_SIZE)
  //   return;

  // dlBuffer[dlBufferIndex++] = (uint8_t)data;
  // dlBuffer[dlBufferIndex++] = (uint8_t)(data >> 8);
  // dlBuffer[dlBufferIndex++] = (uint8_t)(data >> 16);
  // dlBuffer[dlBufferIndex++] = (uint8_t)(data >> 24);
  uint8_t buff[4];
...
void EVE_Write16(uint16_t data) {
  // if (dlBufferIndex + 2 > DL_BUFFER_SIZE)
  //   return;
...
void EVE_Write8(uint8_t data) {
  // if (dlBufferIndex++ > DL_BUFFER_SIZE)
  //   return;

  // dlBuffer[dlBufferIndex++] = (uint8_t)data;
  

  display_SPI_transfer(&data, NULL, 1);
...
uint32_t EVE_Read32(void) {
  ...
  display_SPI_transfer(NULL, rxBuffer, 4);

  uint32_t val = (uint32_t)rxBuffer[0] | ((uint32_t)rxBuffer[1] << 8) |
...
uint16_t EVE_Read16(void) {
  ...
  display_SPI_transfer(NULL, rxBuffer, 2);

  uint32_t val = (uint16_t)rxBuffer[0] | ((uint16_t)rxBuffer[1] << 8);
...
void EVE_AddrForWr(uint32_t ftAddress) {
  ...
  display_SPI_transfer(addr, NULL, 1);
}
