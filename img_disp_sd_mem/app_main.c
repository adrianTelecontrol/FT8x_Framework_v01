...
void EVE_LoadBitmap(void)
{
    uint16_t width = 800;
    uint16_t height = 480;

    API_LIB_WriteDataRAMG(BMP_TEST_1, BMP_TEST_1_SIZE, RAM_G);
...
main(void)
{
    ...
    */
    ...
    UARTprintf("Initial FT81x state... \n");
    ...
    //QuickSanityCheck(); 
    // DiagnosticRawRead((uint32_t)0x00);
    // RunAllSPITests();
    ...
    UARTprintf("FT81x State before loading the image\n");
    EVE_Util_DebugReport();

    SysCtlDelay(MS_2_CLK(1000));
    // EVE_LoadImage();
    EVE_LoadBitmap();
    SysCtlDelay(MS_2_CLK(50000));
