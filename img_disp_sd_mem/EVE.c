...
void EVE_Util_DebugReport(void) {
  ...
  UARTprintf("SPI LINK:      ");
  if (chipID == 0x7C)
    UARTprintf("[OK] (ID: 0x7C)\n");
  else
    UARTprintf("[FAIL] (Read: 0x%02X - Check MISO/MOSI)\n", chipID);

  // Co-Processor Status
  UARTprintf("CO-PROCESSOR:  ");
  if (cmdRead == 0xFFF)
    UARTprintf("[FAULT] (CMD_READ == 0xFFF)\n");
  else if (cpuReset == 1)
    UARTprintf("[RESET] (Engine Held in Reset)\n");
  else if (cmdRead != cmdWrite)
    UARTprintf("[BUSY] (Processing Commands)\n");
  else
    UARTprintf("[IDLE] (Ready)\n");

  // Register Dump
  UARTprintf("  -> CMD_READ:  0x%04X (%d)\n", cmdRead, cmdRead);
  UARTprintf("  -> CMD_WRITE: 0x%04X (%d)\n", cmdWrite, cmdWrite);
  UARTprintf("  -> BUFFER REMAINING: %d bytes\n",
             (4096 - 4) - ((cmdWrite - cmdRead) & 4095));

  // Display Status
  UARTprintf("DISPLAY ENGINE:");
  if (pclk == 0)
    UARTprintf(" [OFF] (PCLK = 0)\n");
  else
    UARTprintf(" [RUNNING] (PCLK = %d)\n", pclk);
  UARTprintf("  -> CMD_DL:    0x%04X (Current Display List Offset)\n", cmdDL);
