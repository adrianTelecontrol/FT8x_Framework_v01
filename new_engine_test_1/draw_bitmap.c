...
void initializeSquareObjects(void) {
    ...
    for (i = 0; i < SQUARE_OBJ_LEN; i++) {
		...
		u16 ui16SideSize = (u16)( rand() % (int)( ( SCREEN_HEIGHT / 4.0f ) - (SCREEN_HEIGHT / 7.0f) + 1.0f) + (SCREEN_HEIGHT / 7.0f) );
		...
		sqr->mass = sqr->scale * sqr->width * sqr->height;
		sqr->inertia = (1.0f / 6.0f) * sqr->mass * (ui16SideSize * ui16SideSize);
		sqr->angularVel = rand() % 2.0f;
    }
}
...
void basicObjectCollision(void) {
    ...
    for (i = 0; i < SQUARE_OBJ_LEN; i++) {
        ...
        // Apply a bit of auto-rotation for visual flair
        a->angle += 1.0f;
        ...
        for (j = i + 1; j < SQUARE_OBJ_LEN; j++) {
            ...
            if (distanceSq < minDistanceSq) {
				...
				f32 ratioA = (f32)a->mass / totalMass;
				f32 ratioB = (f32)b->mass / totalMass;
