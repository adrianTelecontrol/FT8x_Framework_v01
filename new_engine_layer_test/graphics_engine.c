...
// Force inline to avoid branch overhead in the render loop
static inline void Gfx_RestoreBackground_Fast(pixel16_t *pCleanBackground,
                                              pixel16_t *pDrawBuffer, int16_t x,
                                              int16_t y, int16_t width,
                                              int16_t height) {
  // Ensure we don't draw outside the 800x480 screen
  if (x < 0) {
    width += x;
    x = 0;
  }
  ...
  for (row = 0; row < height; row++) {
    // Calculate starting indices for this specific row
    uint32_t offset = ((y + row) * 800) + x;

    // Cast to 32-bit pointers to move TWO pixels per clock cycle
    uint32_t *pSrc32 = (uint32_t *)&pCleanBackground[offset];
    uint32_t *pDst32 = (uint32_t *)&pDrawBuffer[offset];

    // Divide width by 2 because we process 2 pixels at a time
    int16_t colCount = width / 2;

    // Unrolled loop for maximum SDRAM burst speed
    while (colCount--) {
      *pDst32++ = *pSrc32++;
    }

    // Handle the trailing odd pixel if width is an odd number
    if (width % 2 != 0) {
...
void DisplayBitmap(void) {
  ...
  API_CMD_SETBASE(10);
#endif // MEASURE_PERF_ENABLE_FPS
#endif // MEASURE_PERF_ENABLE

  API_COLOR_RGB(255, 0, 0); // Place process duration in ms
...
void Gfx_composite(pixel16_t *pLayer0, pixel16_t *pLayer1)
{
        ...
        for(ui32Index = 0; ui32Index < 800*480; ui32Index++)
        {
                uint16_t ui16Layer0 = pLayer0[ui32Index].u16;
                uint16_t ui16Layer1 = pLayer1[ui32Index].u16;

                if(ui16Layer1 == 0x0000)
                        Gfx_loadIntoBuffer(ui32Index, ui16Layer0);
                else
                        Gfx_loadIntoBuffer(ui32Index, ui16Layer1);
        }
}*/

void Gfx_composite(pixel16_t *pLayer0, pixel16_t *pLayer1) {
  // Cast 16-bit pixel pointers to 32-bit pointers
  // This allows us to fetch TWO pixels per SDRAM read/write
  uint32_t *pSrc0 = (uint32_t *)pLayer0;
  uint32_t *pSrc1 = (uint32_t *)pLayer1;
  uint32_t *pDest =
      (uint32_t *)g_pDrawingBuffer; // Write directly, no function call

  // We process 2 pixels per iteration, so divide total pixels by 2
  uint32_t ui32LoopCount = (800 * 480) / 2; // 192,000 iterations

  while (ui32LoopCount--) {
    // Fetch 2 pixels from Layer 0 and 2 pixels from Layer 1
    uint32_t val0 = *pSrc0++;
    uint32_t val1 = *pSrc1++;
    uint32_t outPixelPair = 0;

    // --- Evaluate Pixel 1 (Lower 16 bits) ---
    // If Layer1's pixel is 0x0000 (transparent), use Layer0's pixel
    ...
    // --- Evaluate Pixel 2 (Upper 16 bits) ---
    ...
    // Write the 2 blended pixels to SDRAM in a single 32-bit transaction
    *pDest++ = outPixelPair;
  }
}
