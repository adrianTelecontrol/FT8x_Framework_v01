...
extern uint32_t g_ui32SysClock; // For delays

void EVE_Write32(uint32_t data) {
  uint8_t buff[4];

  buff[0] = (uint8_t)data;
  buff[1] = (uint8_t)(data >> 8);
  buff[2] = (uint8_t)(data >> 16);
  buff[3] = (uint8_t)(data >> 24);

  display_SPI_transfer(buff, NULL, 4);
}

void EVE_Write16(uint16_t data) {
  uint8_t buff[2];

  buff[0] = (uint8_t)data;
  buff[1] = (uint8_t)(data >> 8);
  display_SPI_transfer(buff, NULL, 2);
}

void EVE_Write8(uint8_t data) { display_SPI_transfer(&data, NULL, 1); }
...
uint32_t EVE_Read32(void) {
  ...
  display_SPI_transfer(&txBuffer, &rxBuffer[0], 1);
  display_SPI_transfer(&txBuffer, &rxBuffer[1], 1);
  display_SPI_transfer(&txBuffer, &rxBuffer[2], 1);
  display_SPI_transfer(&txBuffer, &rxBuffer[3], 1);

  val = (uint32_t)rxBuffer[0] | ((uint32_t)rxBuffer[1] << 8) |
...
uint16_t EVE_Read16(void) {
  ...
  uint16_t val = (uint16_t)rxBuffer[0] | ((uint16_t)rxBuffer[1] << 8);
...
uint8_t EVE_Read8(void) {
  ...
  display_SPI_transfer(&txBuffer, &rxBuffer, 1);
...
void EVE_Flush_Buffer(void) {
  ...
  // 1. Wait for space in the EVE FIFO
  //    We need to ensure EVE has processed enough previous commands.
  uint16_t spaceAvailable;
  do {
    uint16_t readPtr =
        EVE_MemRead16(REG_CMD_READ); // You might need to implement
                                     // EVE_MemRead16 using EVE_Write8
    uint16_t fullness = (cmdOffset - readPtr) & 4095;
    spaceAvailable = (4096 - 4) - fullness;
  } while (spaceAvailable < dlBufferIndex);

  // 2. Calculate if we need to wrap around the ring buffer
  uint16_t chunk1 = dlBufferIndex;
  uint16_t chunk2 = 0;

  if (cmdOffset + dlBufferIndex > 4096) {
    // Split transfer required
    chunk1 = 4096 - cmdOffset;
    chunk2 = dlBufferIndex - chunk1;
  }
  ...
  // Send Address Header (RAM_CMD + cmdOffset)
  uint32_t writeAddr = RAM_CMD + cmdOffset;
  uint8_t addrHeader[3];
  addrHeader[0] = (uint8_t)((writeAddr >> 16) | 0x80); // Write Flag
  addrHeader[1] = (uint8_t)(writeAddr >> 8);
  addrHeader[2] = (uint8_t)(writeAddr);

  // Send Header (Blocking, but short)
  display_SPI_transfer(addrHeader, NULL, 3);

  // Send Data (DMA - Blocking wait)
  display_SPI_transfer(dlBuffer, NULL, chunk1);

  EVE_CS_HIGH();

  // ---------------------------------------------------------
  // TRANSFER CHUNK 2 (If Wrapped)
  // ---------------------------------------------------------
  if (chunk2 > 0) {
    EVE_CS_LOW();

    // Wrap Address: RAM_CMD + 0
    writeAddr = RAM_CMD;
    addrHeader[0] = (uint8_t)((writeAddr >> 16) | 0x80);
    addrHeader[1] = (uint8_t)(writeAddr >> 8);
    addrHeader[2] = (uint8_t)(writeAddr);

    display_SPI_transfer(addrHeader, NULL, 3);

    // Send the rest of the buffer (offset by chunk1)
    display_SPI_transfer(&dlBuffer[chunk1], NULL, chunk2);

    EVE_CS_HIGH();
  }

  // 3. Update the Write Pointer in EVE
  //    We modified the RAM, now tell EVE "I wrote X bytes"
  cmdOffset = (cmdOffset + dlBufferIndex) & 4095;

  // We update REG_CMD_WRITE. This triggers the EVE coprocessor to start
  // working.
  EVE_MemWrite16(REG_CMD_WRITE, cmdOffset);

  // 4. Reset Local Buffer
  dlBufferIndex = 0;
}
...
void EVE_AddrForWr(uint32_t ftAddress) {
  ...
  addr[0] = (uint8_t)((ftAddress >> 16) | MEM_WRITE);
  addr[1] = (uint8_t)(ftAddress >> 8);

  addr[2] = (uint8_t)(ftAddress);

  display_SPI_transfer(addr, NULL, 3);
}
...
void EVE_AddrForRd(uint32_t ftAddress) {
  ...
  addr[0] = (uint8_t)((ftAddress >> 16) | MEM_READ);
  addr[1] = (uint8_t)(ftAddress >> 8);
  ...
  display_SPI_transfer(addr, NULL, 4);
}

// -------------- Write a 32-bit value to specified address --------------------
void EVE_MemWrite32(uint32_t ftAddress, uint32_t ftData32) {
  uint8_t buff[4];
  EVE_CS_LOW(); // CS low begins the SPI transfer

  EVE_AddrForWr(ftAddress); // Send address to be written

  buff[0] = (uint8_t)ftData32;
  buff[1] = (uint8_t)(ftData32 >> 8);
  buff[2] = (uint8_t)(ftData32 >> 16);
  buff[3] = (uint8_t)(ftData32 >> 24);

  display_SPI_transfer(buff, NULL, 4);

  EVE_CS_HIGH(); // CS high terminates the SPI transfer
}
...
void EVE_MemWrite16(uint32_t ftAddress, uint16_t ftData16) {
  ...
  buff[0] = (uint8_t)ftData16;
  buff[1] = (uint8_t)(ftData16 >> 8);
  display_SPI_transfer(buff, NULL, 2);
...
void EVE_MemWrite8(uint32_t ftAddress, uint8_t ftData8) {
  ...
  display_SPI_transfer(&ftData8, NULL, 1);
...
void EVE_CmdWrite(uint8_t EVECmd, uint8_t Param) {
  ...
  //    MCU_Delay(10);
  display_SPI_transfer(&EVECmd, NULL, 1);
  display_SPI_transfer(&Param, NULL, 1);
  display_SPI_transfer(&frameEnd, NULL, 1);
  ...
  do {
    ...
    WritePointer = EVE_MemRead16(
        REG_CMD_WRITE); // Read the graphics processor write pointer
           ...
           (ReadPointer != 0xFFF)); // && (countFifoUnresponded >= 20)); // Wait
                                    // until the two registers match
...
void EVE_Init(void) {
  ...
  while (EVE_MemRead8(REG_ID) != FT81x_CHIP_ID_VALUE) {
    UARTprintf("Error! FT81x ID mismatch! Retrying... \n");
    SysCtlDelay(MS_2_CLK(100));
  }
  UARTprintf("REG_ID is correct! \n");
...
void API_WakeUpScreen(void) {
  ...
  EVE_PD_LOW();
  SysCtlDelay(MS_2_CLK(50)); // 350
  EVE_PD_HIGH();
  SysCtlDelay(MS_2_CLK(50)); // Must be at least 20ms
...
void API_LIB_EndCoProList(void) {
  EVE_CS_HIGH(); // AVS                                                 // Chip
                 // Select high concludes burst
...
void API_LIB_WriteDataToCMD(const uint8_t *ImgData, uint32_t TotalDataSize) {
  ...
  // If the data is more than 1000 bytes, it is sent as a series of one or more
  // bursts and then the remainder. It uses a target value which specifies the
  // last index (of the source data) for each chunk e.g. if sending 1500 bytes,
  // we set the target to 999 first and send 1000 bytes (image data index 0 to
  // 999). Then we set the target to 1499 and send the remainder of the data
  // from image data index 1000 to 1499.
  ...
  while (CurrentIndex < EndIndex) // While not all data is sent
  {
    ...
    {
      Target =
          EndIndex; // ... then add the amount of data to the current target
    ...
    if (IsLastChunk != 0) // If this is the last chunk of the data,
    {
      ...
      CurrentIndex++;
      CurrentChunk++;
      // ... and then pad with 00s if necessary to make overall data size a
      // multiple of 4 bytes
