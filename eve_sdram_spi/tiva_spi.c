...
static const uint8_t g_dummyTxZero = 0;
...
bool display_SPI_transfer(const uint8_t *pTxBuffer, uint8_t *pRxBuffer, uint32_t count)
{
    ...
    // If you skip this, the DMA logic might not reset for the new size (1 vs 2 bytes)
    ...
    // Without this, the DMA reads old data immediately and finishes early (or hangs).
    uint32_t garbage;
