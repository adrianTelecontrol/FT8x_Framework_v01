...
bool gfx_initRegTouch(void *widget, widget_type_e type) {
  if (type == WD_TYPE_BUTTON) {
    ...
    btn->regTouch.x1 = btn->pos.x;
    btn->regTouch.x2 = btn->pos.x + btn->dim.width;
    btn->regTouch.y1 = btn->pos.y;
    btn->regTouch.y2 = btn->pos.y + btn->dim.height;

    return true;
  }
  else if (type == WD_TYPE_RECT) {
    Button *btn = (Rectangle *)widget;
    btn->regTouch.x1 = btn->pos.x;
    btn->regTouch.x2 = btn->pos.x + btn->dim.width;
    btn->regTouch.y1 = btn->pos.y;
    btn->regTouch.y2 = btn->pos.y + btn->dim.height;
