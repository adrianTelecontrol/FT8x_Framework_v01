...
void MCU_PDhigh(void) {
  GPIOPinWrite(PUERTO_M, SPI_PD_PANTALLA, HIGH);
...
void APP_Init(void) {
  ...
  uint32_t ramCommandBuffer =
      RAM_CMD;        // Set beginning of graphics command memory *
  ...
  // would be called here before the Active command wakes up the device
