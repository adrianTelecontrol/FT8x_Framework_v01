...
/*
void Gfx_SendContinuousBlock_Blocking(pixel16_t *pActiveBuffer, int16_t x, int16_t y, int16_t w, int16_t h) {
    // 1. Bounds checking
    if (x < 0) { w += x; x = 0; }
    if (y < 0) { h += y; y = 0; }
    if (x + w > 800) w = 800 - x;
    ...
    if (g_RenderEngine.state == RENDER_IDLE) 
	{
      ...
      if (simulateTouch) {
        ...
        btn.state = (btn.state == BTN_STATE_NORMAL) ? BTN_STATE_PRESSED
      	...
      	Gfx_RestoreBackground_Fast((pixel16_t *)sBitmapHandler.ui8Pixels,
                                 g_pDrawingBuffer, LCD_WIDTH / 2 - 150, LCD_HEIGHT / 2 - 100, 140, 90);
		...
		// Gfx_SendContinuousBlock_Blocking(g_pDrawingBuffer, bboxX, bboxY, bboxW, bboxH);

        Gfx_SendContinuousBlock_SG(g_pDrawingBuffer, bboxX, bboxY, bboxW, bboxH);
      }
