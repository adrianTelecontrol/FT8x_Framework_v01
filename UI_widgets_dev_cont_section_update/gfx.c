...
void gfx_drawButton(pixel16_t *pBuf, gfx_Button *btn)
{
    ...
    if(btn->borderWidth) {
        gfx_fillRoundRect(pBuf, 
                          btn->pos.x - btn->borderWidth, 
                          btn->pos.y - btn->borderWidth, 
                          btn->size.width + btn->borderWidth*2, 
                          btn->size.height + btn->borderWidth*2, 
                          btn->radius, 
                          activeBorderColor);
    }

    // 2. Draw Background (Using the active background color)
    gfx_fillRoundRect(pBuf, btn->pos.x, btn->pos.y, btn->size.width, btn->size.height, btn->radius, activeBgColor);
    
    // 3. Draw Label (Using the active text color)
    gfx_DrawString(pBuf, btn->font, btn->pos.x + btn->size.width / 2.0f, btn->pos.y + btn->size.height / 2.0f, 
