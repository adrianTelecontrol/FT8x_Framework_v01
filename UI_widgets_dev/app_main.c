...
int main(void) {
  ...
  *pCheck = 0xDEADBEEF;
  ...
  while (1) {
    ...
    gfx_DrawString(g_pDrawingBuffer, FONT_BEBAS, 300, 400, text, 0xFD20, 2, ALIGN_CENTER);
	...
	gfx_Button btn = (gfx_Button){
		...
		.font = FONT_BEBAS,
		.fontScale = 2,
		.size = (Size){.width = 120*2, .height = 80*2},
    ...
    Gfx_setProcessDuration((ui32ProcEnd - ui32ProcStart) / ui32CyclesPerMs);
