...
void API_LIB_WriteDataRAMG_uDMA(const uint8_t *pui8ImgSrc,
  ...
  while (ui32BytesSent < ui32Size) {
    ...
    // Manual Copy Loop (Optimized for Stability over Speed)
    uint32_t *pSrc32 = (uint32_t *)&pui8ImgSrc[ui32BytesSent];
    uint32_t *pDest32 = (uint32_t *)pui8Buffer;
    uint32_t wordsToCopy = ui32CurrChunkSize / 4;
    uint32_t i;

    for (i = 0; i < wordsToCopy; i++) {
      // 1. Read from SDRAM
      uint32_t val = pSrc32[i];

      // 2. Write to SRAM buffer
      pDest32[i] = val;

      // 3. THE MAGIC FIX: Artificial Delay
      // We add a tiny delay to break the "Burst" access.
      // This gives the SDRAM capacitors time to recover between reads,
      // exactly like the Polling function did naturally.
      SysCtlDelay(2); // ~6 cycles delay. Adjust if needed.
    }

    // Handle remaining bytes (if chunk size isn't div by 4)
    for (i = wordsToCopy * 4; i < ui32CurrChunkSize; i++) {
      pui8Buffer[i] = pui8ImgSrc[ui32BytesSent + i];
    }
