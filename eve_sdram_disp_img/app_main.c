...
int main(void) {
  ...
  *pCheck = 0xDEADBEEF;
  ...
  // Transfer the bitmap data to the SDRAM
  uint8_t *g_ui32BitmapSDRAM =
      (uint8_t *)malloc(sizeof(uint8_t) * BMP_TEST_1_SIZE);
  // memset(g_ui32BitmapSDRAM, 0x22, BMP_TEST_1_SIZE);
  ...
  if (ret) {
    ...
    int iFetchFileStatus =
        SDSPI_FetchBitmap(g_BMP_FILENAME, &sBitmapHandler, BMP_TEST_1_SIZE);
  }
  /*
  // Verify data integrity
  uint32_t ui32Index = 0;
  for (ui32Index = 0; ui32Index < BMP_TEST_1_SIZE; ui32Index++) {
    if (g_ui32BitmapSDRAM[ui32Index] != BMP_TEST_1[ui32Index])
      break;
  }
  if (ui32Index != BMP_TEST_1_SIZE)
    TIVA_LOGE(TASK_NAME, "Error copying bitmap to SDRAM in index %d",
              ui32Index);
  else
    TIVA_LOGI(TASK_NAME, "Bitmap copied sucessfully to SDRAM");
        */
  // Setting up the Ft81x
  ...
  SysCtlDelay(MS_2_CLK(1000));
  TIVA_LOGI(TASK_NAME, "Starting image loading...");
  uint8_t ui8BitmapFlags = 0;
  //ui8BitmapFlags |= EVE_LOAD_IMG_POLLING;
  ui8BitmapFlags |= EVE_LOAD_IMG_UDMA;
  // BitmapHandler_t sConstBmpHandler;
  // sConstBmpHandler.ui8Pixels = (uint8_t *)&BMP_TEST_1[0x7A];
  // sConstBmpHandler.sHeader.bitmap_width = 719;
  // sConstBmpHandler.sHeader.bitmap_height = 480;
  // EVE_LoadBitmap(&sConstBmpHandler, BMP_TEST_1_SIZE, ui8BitmapFlags);
  EVE_LoadBitmap(&sBitmapHandler, ui8BitmapFlags);
  // EVE_LoadBitmap(BMP_TEST_1, BMP_TEST_1_SIZE, ui8BitmapFlags);

  SysCtlDelay(MS_2_CLK(2000));
  const uint16_t width = 800;
  const uint16_t height = 480;

  uint32_t ui32Counter = 0;
  while (1) {
    TIVA_LOGI(TASK_NAME, "Running...");
    ...
    API_CMD_NUMBER(LCD_WIDTH - 100, LCD_HEIGHT - 50, 30, 0, ui32Counter);
    API_CMD_SETBASE(10);
